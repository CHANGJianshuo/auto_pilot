...
| [0076](./0076-figure-eight.md) | 2026-04-23 | M17 figure-8 轨迹 SITL | `c6de302` | `sim_hil::sitl::figure_eight(A, T, alt)` lemniscate of Gerono + 解析 velocity/accel FF；新 `run_with_controller_trajectory` + `TrajectorySitlResult { rms, max_err, vel_rms }`；3 测（self-consistent 数学 / MPC-I RMS < 0.5 m / MPC-I 胜 PI 在动态目标下）；workspace 248→251；Phase III 首个基准落地，给 NN residual 提供具象数字。|
| [0077](./0077-motor-failure.md) | 2026-04-23 | M18 单电机失效 SITL | `705e399` | `SimConfig.motor_fault_mask: [f32; 4]` 默认 `[1.0; 4]`；在 motor lag **之后**乘 mask；新测 `single_motor_failure_state_stays_bounded`（motor 0 在 t=2s 死亡，MPC-I 控制 3s，断言位置/速度 finite、高度 err < 60m、tilt < 170°）；algo-alloc 目前 4-motor 固定 E_inv 会失效 → vehicle 下落，承认是"honest baseline"，M19 补 3-motor failover allocation 后断言收紧到 ± 2m；workspace 251→252。|
| [0078](./0078-failover-allocator.md) | 2026-04-23 | M19 3-motor 失效切换 | `3009838` + `6f8267c` + `6cbd8eb` | **M19a**：`algo-alloc::FailoverAllocator` 查表 4 套 3-motor 伪逆（Mueller & D'Andrea 2014，放弃 yaw），5 新测（含 proptest）。**M19b**：`RateLoopConfig.failover: Option<FailoverAllocator>` + `FlightState.motor_alive: [bool; 4]`；rate_loop_step 只在 motor_alive 非全 true 时切换，零破坏现有测。**M19c**：M18 SITL 收紧 altitude err 60→3 m、tilt 170°→60°（oracle 模式 `motor_alive[0]=false`）。整体把"honest baseline"升级成真 failover regression guard。|
| [0079](./0079-motor-fault-detector.md) | 2026-04-23 | M20 MotorFaultDetector end-to-end | `074b3d5` + `1426980` + `0ecef90` | **M20a**：`algo-fdir::MotorFaultDetector` 通过比较 cmd-predicted ω̇ 和 observed ω̇，用 torque-direction alignment score 定位失效电机；6 新测（含旋转对称四个电机都能检出）。**M20b**：`rate_loop_step` 挂上 detector + AND-monotone 更新 `flight.motor_alive`（保留 M19c oracle 模式 compat）+ 新字段 `effectiveness` `inertia_inv` `last_motor_cmd`。**M20c**：SITL 端到端 —— 无 oracle，detector 在 < 250 ms 内自己找到死电机并触发 failover，altitude err < 5 m、tilt < 75°。Phase II+ 真机硬件上**不用改一行**就能跑。|
| [0080](./0080-motor-fault-detector-kani.md) | 2026-04-24 | M20d Kani on MotorFaultDetector | `3eed5b4` | 把 `observe()` 的 f32 决策和 state-machine 拆开：新 `pub enum DetectorDecision { Quiet / Inconclusive / BestMatch(u8) }`，Kani 证 `apply_decision` 纯状态转移。6 新 proof（latch monotone / level monotone / dead_count ≤ 4 / Quiet 零化 persistence / Inconclusive 不动状态 / BestMatch 只保留 idx 号 counter）。Workspace 形式化证明 **28 → 34**。套路和 M1.14 / M16a–c 同：关键状态决策剖离 f32 即 Kani-amenable。|
| [0081](./0081-three-motor-yaw-handoff.md) | 2026-04-24 | M21 3-motor yaw handoff | `a50d0d5` | `outer_step` 在 `motor_alive != [true;4]` 时：`active_sp.yaw_rad` 被覆盖为 `flight.state.attitude` 的当前 yaw（Hamilton atan2）→ `q_desired` yaw 和 `q_current` 一致 → `attitude_to_rate` 自然返回 yaw_rate = 0；belt-and-suspenders 显式把 `rate_cmd.body_rate_rad_s.z = 0`。M19c SITL 测试新增 `max_xy_err < 3 m` 断言（pre-M21 会无限漂）。Mueller 论文的 "yaw 为自由状态" 的 80% 效果，用 < 50 行代码做到。|
| [0082](./0082-roll-flip.md) | 2026-04-24 | M22 Roll flip SITL（Phase III #3）| `c48dae8` | `roll_flip_completes_and_recovers_upright`：1s hover + 0.5s body_rate=(15,0,0) + 1.5s recovery via `attitude_to_rate(q_identity)`；断言 min q_w < 0（实际翻 inverted）、final tilt < 30°、altitude loss < 15 m、‖q‖ 稳定。INDI 裸飞，无 explicit flip mode。副产物：`FlightState.motor_fault_detector_enabled: bool`（默认 true；acro maneuvers 下 saturate 会让 ω̇ residual 超 detector threshold，mode-gated 关掉它）。plan.md Phase III #3 落地；#4 Swift 类 gate-pass 仍是 stretch goal（需训练 NN）。|
| [0083](./0083-statustext-mavlink.md) | 2026-04-24 | MAVLink STATUSTEXT encoder | `b4a4b8f` | `comms-mavlink::encode_statustext(sys, comp, seq, severity, text)` + `pub use MavSeverity`；MAVLink 50-byte 固定文本 + severity 枚举；null-pad 尾、> 49 字符截断到 49 + null。2 新 test（round-trip + truncation）。Pilot-facing 警报的规范通道："MOTOR 0 FAILED" / "PREFLIGHT REJECTED" 等，QGC 原生 toast 显示。M23b 计划把这个挂到 `sim-hil::sitl_mavlink` runner 里，detector 宣告 → UDP broadcast。|
